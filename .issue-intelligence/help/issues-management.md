...
{
  "issueNumber": 1,
  "sessionPath": ".issue-intelligence/state/sessions/2026-02-04T12-00-00-000Z_abc123.jsonl",
...
tail -5 .issue-intelligence/state/sessions/*.jsonl

# Search all sessions for a keyword
grep -r "search term" .issue-intelligence/state/sessions/
```
...
4. The result is saved to `.issue-intelligence/AGENTS.md`

Hatching is optional — the agent works without it, but it's more fun with a personality.

## What You Can Ask

Issue Intelligence can handle a wide range of tasks through issues:
...
```bash
rm .issue-intelligence/state/issues/<number>.json
# Find and delete the corresponding session file
rm .issue-intelligence/state/sessions/<timestamp>_<id>.jsonl
...
git add -A
git commit -m "Clear all issue-intelligence sessions"
git push
```
