# ⚙️ Configure Issue Intelligence
...
| File | Purpose |
|------|---------|
| `.issue-intelligence/.pi/settings.json` | LLM provider, model, and thinking level |
| `.issue-intelligence/AGENTS.md` | Agent identity — name, personality, instructions |
| `.issue-intelligence/.pi/APPEND_SYSTEM.md` | System prompt loaded every session |
| `.issue-intelligence/.pi/BOOTSTRAP.md` | First-run identity prompt (used during hatching) |
| `.issue-intelligence/.pi/skills/` | Modular skill packages |
...
Edit `.issue-intelligence/.pi/settings.json`:
...
Edit `.issue-intelligence/AGENTS.md` to define the agent's identity, name, personality, and behavioral instructions. This file is read at the start of every session.
...
Edit `.issue-intelligence/.pi/APPEND_SYSTEM.md` to change the system-level instructions loaded on every session. This file controls:
...
Skills are modular capability packages stored as Markdown files in `.issue-intelligence/.pi/skills/`. Each skill defines specific behaviors, instructions, or capabilities the agent can use.
...
```bash
git add -A
git commit -m "Update issue-intelligence configuration"
git push
```
